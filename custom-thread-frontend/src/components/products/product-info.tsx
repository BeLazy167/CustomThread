...
export default function ProductInfo() {
  ...
  const [cartItems, setCartItems] = useState([
    { id: 1, name: "UltraComfort Tee", price: 29.99, image: "/placeholder.jpg", size: "M", quantity: 1 },
    { id: 2, name: "Premium Joggers", price: 49.99, image: "/placeholder.jpg", size: "L", quantity: 2 },
  ]);
  ...
  return (
    <div className="sticky top-8 space-y-6 max-w-md mx-auto">
      ...
      <div className="flex flex-col gap-3">
        <Button size="lg" className="w-full bg-black text-white hover:bg-black" onClick={handleAddToCart}>
          Add to bag
        </Button>
        <Button
          size="lg"
      ...
      <p className="mt-3 text-gray-600 dark:text-gray-300 text-sm leading-relaxed">
        Experience ultimate comfort with our UltraComfort Tee. Crafted from premium breathable cotton, this tee offers a soft feel, 
        relaxed fit, and durability for everyday wear. Perfect for casual outings, gym sessions, or just lounging in style.
      </p>
      ...
      <Sheet open={isCheckoutOpen} onOpenChange={setIsCheckoutOpen}>
        <SheetContent className="w-full sm:max-w-lg flex flex-col h-screen backdrop-blur-lg bg-white/80 dark:bg-black/60">
          <SheetHeader>
            <SheetTitle className="text-lg font-semibold">Your Shopping Bag</SheetTitle>
          </SheetHeader>

          <div className="flex-grow overflow-y-auto mt-6 space-y-6 px-4">
            {cartItems.map((item) => (
              <div key={item.id} className="flex items-center gap-4">
                <img src={item.image} alt={item.name} className="h-24 w-20 rounded-lg object-cover border" />
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-4 mt-2">
                    ...
                    />
                    <span className="text-lg font-semibold min-w-[32px] text-center">{item.quantity}</span>
                    <PlusIcon
                      size={22}
