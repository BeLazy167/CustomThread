...
export default function LivePackageJourney() {
  ...
  return (
    <div className="max-w-7xl mx-auto mt-10">
      <Card>
        <CardHeader>
          <CardTitle className="text-xl font-bold text-center">Live Package Journey 📦</CardTitle>
        </CardHeader>
        <CardContent>
          ...
          <div className="flex justify-between mt-6 px-12 w-full max-w-7xl mx-auto">
            ...
            return (
                <div key={stage.id} className="flex flex-col items-center space-y-2 mx-6">
                  <Icon
                    className={`h-10 w-10 ${index <= currentStage ? "text-green-500" : "text-gray-400 dark:text-gray-500"}`}
                  />
                  <p className={`text-base font-medium mt-2 ${index <= currentStage ? "text-black dark:text-white" : "text-gray-500"}`}>
                      {stage.label}
                  </p>
                </div>
                );
